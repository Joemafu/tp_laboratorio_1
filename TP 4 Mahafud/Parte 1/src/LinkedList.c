...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if((this!=NULL)&&(nodeIndex>=0)&&(nodeIndex<=ll_len(this)))///NO validar que pElement no sea NULL porque el test corre con elementos NULL
    {
...
int ll_add(LinkedList* this, void* pElement)
{
    ...
    if(this!=NULL)///NO validar que pElement no sea NULL porque el test corre con elementos NULL
    {
        len=ll_len(this);
...
int ll_remove(LinkedList* this,int index)
{
    ...
    if((this!=NULL)&&(index>=0)&&(index<(len=ll_len(this))))
    {
...
int ll_deleteLinkedList(LinkedList* this)
{
    ...
    {
        ll_clear(this);
        this=NULL;//free(this);///Tuve que anular el free porque sino cuando una función queria mostrar un linked list que había sido borrado, la app mostraba basura en loop al no reconocer el valor del size.
        returnAux=0;
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
    ...
    if(this!=NULL&&from>=0&&from<to)
    {
        ...
        if(len>from&&to<=len)
        {
            ...
            cloneArray->size=0;
            cloneArray->pFirstNode=NULL;
            if (cloneArray!=NULL)
            {
                for(i=0;i<to-from;i++)
                {
                    ll_add(cloneArray,ll_get(this,from+i));
                }
            }
        }
    }
    ...
    if(this!=NULL&&pFunc!=NULL&&(order==1||order==0))
    {
        ...
        for(i=0;i<len-1;i++)
        {
            for(j=i+1;j<len;j++)
            {
                ...
                }else if(order==1&&pFunc(pElementI,pElementJ)>0)
                {
                    pElementAux=pElementI;
                    ll_set(this,i,pElementJ);
                    ll_set(this,j,pElementAux);
